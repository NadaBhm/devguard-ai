...
class TestSafeNodeWrapper:
    def test_catches_exception(self, initial_state):
        ...
        result = safe_node_wrapper(failing_node, "test_node", initial_state)
    ...
    def test_skips_if_already_failed(self, initial_state):
        ...
        result = safe_node_wrapper(should_not_run, "skipped", initial_state)
        assert result["status"] == "failed"
...
class TestSafeNodeWrapperWithInterrupt:
    def test_graph_interrupt_is_re_raised(self, initial_state):
        ...
        def node_with_interrupt(state):
            # langgraph >=1.x: GraphInterrupt takes `interrupts` (a sequence
    ...
    def test_real_exception_is_caught(self, initial_state):
        ...
        result = safe_node_wrapper(node_with_error, "error_node", initial_state)
