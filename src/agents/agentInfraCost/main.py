...
def _mock_artifacts(analysis: RepoAnalysisInput, decision: DecisionResult) -> Artifacts:
    """Build the artifacts block — Terraform is now genuinely generated by
    module 3 (``terraform_generator``); the docker fields follow the
    contract's 'commit_sha if available, else "latest" + warning' rule.
    """
    ...
    terraform = TerraformArtifacts(
        ...
        variables={"region": terraform_context.region, "environment": terraform_context.environment},
    )
